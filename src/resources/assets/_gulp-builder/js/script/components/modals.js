...
export let modals = {
     ...
     */
    base: function(confirm_function, title, content, btn_name, btn_cancel_name = '') {
    ...
    withFrom: function(confirm_function, content) {
        ...
        var error_function = function(errors) {
            ...
            $.map(errors, function(val, name) {
                ...
                $(modal_blk).find('.status').html('<div class="alert alert-danger alert-dismissible"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">?</button><h4><i class="icon fa fa-ban"></i> �������� ������</h4>' + errors_string + '</div>');
            });
        };
     ...
     * @param btn_name
     */
    set_content: function(modal_blk, title, content, btn_name, btn_cancel_name) {
        ...
        if(btn_cancel_name !== '' ){
            console.log( $(modal_blk).find('.js_confirm_btn_cancel'));
